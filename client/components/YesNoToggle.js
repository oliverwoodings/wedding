...
export default function YesNoToggle ({ toggled, onToggle, className, maybe }) {
  return (
    <div className={classnames(styles.container, className, {
      ...
      <div className={styles.toggle} onClick={toggle(true)}>Yes</div>
      {maybe && <div className={styles.toggle} onClick={toggle(null)}>Maybe</div>}
      <div className={styles.toggle} onClick={toggle(false)}>No</div>
    </div>
  )
